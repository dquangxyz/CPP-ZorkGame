...
Passage::Passage(const std::string& name, const std::string& description, Room* from, Room* to)
        : Location(name, description), fromRoom(from), toRoom(to) {
    setEnterCommand(std::make_shared<PassageDefaultEnterCommand>(this));
}

Passage::Passage(const std::string& name, const std::string& description, std::shared_ptr<Command> enterCommand, Room* from, Room* to)
        : Location(name, description, std::move(enterCommand)), fromRoom(from), toRoom(to) {}
